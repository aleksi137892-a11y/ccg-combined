...
import { universalJurisdictionMapData, type MapJurisdiction } from "@/data/sanctionsMapData";

const VIEWBOX_WIDTH = 960;
const VIEWBOX_HEIGHT = 520;

interface UniversalJurisdictionMapProps {
  className?: string;
}

export const UniversalJurisdictionMap = ({ className }: UniversalJurisdictionMapProps) => {
  ...
  const jurisdictionByCountry = useMemo(() => {
    return new Map(
      universalJurisdictionMapData.jurisdictions.map((jurisdiction) => [
  ...
  const caption = useMemo(
    () =>
      universalJurisdictionMapData.jurisdictions
    ...
    <div className={cn("relative", className)}>
      <div className="relative bg-navy/50 border border-white/10 overflow-hidden">
        <svg
          ...
          style={{ minHeight: 320 }}
          role="img"
          aria-label={isGeorgian ? 'უნივერსალური იურისდიქციის რუკა - აირჩიეთ ქვეყანა დეტალებისთვის' : 'Universal jurisdiction map - select a country for details'}
        >
          <defs>
            <pattern
              id="universal-grid"
            ...
            <filter id="universal-glow" x="-30%" y="-30%" width="160%" height="160%">
              <feDropShadow
                dx="0"
                dy="0"
                stdDeviation="3"
          ...
          <rect width={VIEWBOX_WIDTH} height={VIEWBOX_HEIGHT} fill="url(#universal-grid)" />
            ...
            {features.map((feature, index) => {
              ...
              return (
                <path
                  ...
                  filter={isSelected ? "url(#universal-glow)" : undefined}
                />
              );
            })}
          </g>
...
export default UniversalJurisdictionMap;
