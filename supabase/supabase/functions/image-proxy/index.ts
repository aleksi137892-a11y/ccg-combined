  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (res.status === 429 || (res.status >= 500 && res.status <= 599)) {
        ...
        console.log(
          `image-proxy upstream ${res.status}; attempt ${attempt}/${maxAttempts}; waiting ${waitMs}ms: ${url}`
        );
        res.body?.cancel();
      ...
      console.log(
        `image-proxy fetch error; attempt ${attempt}/${maxAttempts}; waiting ${waitMs}ms: ${url}`
      );
      await new Promise((r) => setTimeout(r, waitMs));
    }
  }
    ...
    try {
      upstream = await fetchWithRetry(
        target.toString(),
        {
          ...
          method,
          headers: {
            // Some hosts (incl. Wikimedia) are picky about headers/UAs.
            "User-Agent":
      ...
      console.error("image-proxy upstream fetch error:", e);
